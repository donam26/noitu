...
export const quizAPI = {
  ...
  getRandomQuestion: async (params = {}) => {
    try {
      ...
      // Nếu params là object và có thuộc tính exclude
      if (typeof params === 'object' && params !== null) {
        if (params.exclude !== undefined) {
          requestData.exclude = Array.isArray(params.exclude) ? params.exclude : [params.exclude];
        }
      } 
      // Nếu params là mảng, coi như là exclude
      else if (Array.isArray(params)) {
        requestData.exclude = params;
      }
      
      console.log("Gửi request lấy câu hỏi ngẫu nhiên với data:", requestData);
      
      const response = await api.post('/quiz/random', requestData);
      return {
        success: response.data.success,
        data: response.data.data,
        message: response.data.message
  ...
  // Gửi kết quả trò chơi - Lưu vào localStorage
  submitGameResult: async (gameStats) => {
    try {
      // Lưu kết quả vào localStorage thay vì gửi API
...
export const knowledgeAPI = {
  ...
  getRandomQuestion: async (exclude = []) => {
    try {
      ...
      // Xử lý tham số exclude
      if (exclude) {
        if (Array.isArray(exclude)) {
          requestData.exclude = exclude.filter(id => id !== undefined && id !== null);
        } else if (typeof exclude === 'number' || typeof exclude === 'string') {
          requestData.exclude = [exclude];
        } else {
          requestData.exclude = [];
        }
      } else {
      ...
      console.log("Gửi request lấy câu hỏi kiến thức với data:", requestData);
      
      const response = await api.post('/knowledge/random', requestData);
      console.log("Nhận phản hồi từ API knowledge:", response.data);
      ...
      console.error("Lỗi khi lấy câu hỏi kiến thức:", error);
      if (error.response) {
  ...
  // Gửi kết quả trò chơi - Chỉ để tương thích với code hiện tại
  submitGameResult: async (gameStats) => {
    try {
      // Lưu kết quả vào localStorage vì không có endpoint API
      const gameHistory = JSON.parse(localStorage.getItem('knowledgeGameHistory') || '[]');
      gameHistory.push({
        ...gameStats,
        timestamp: new Date().toISOString()
      });
      localStorage.setItem('knowledgeGameHistory', JSON.stringify(gameHistory));
...
export const behaviorAPI = {
  ...
  getRandomQuestion: async (exclude = []) => {
    try {
      ...
      console.log("Gửi request lấy câu hỏi behavior với data:", requestData);
      
      const response = await api.post('/behavior/random', requestData);
      return {
        success: response.data.success,
        data: response.data.data,
        message: response.data.message
  ...
  // Xáo trộn các lựa chọn của câu hỏi
  shuffleOptions: async (question) => {
    try {
      const response = await api.post('/behavior/shuffle-options', { question });
      return {
        success: response.data.success,
        data: response.data.data,
        message: response.data.message
      ...
      console.error("Lỗi khi xáo trộn lựa chọn:", error);
      return handleError(error);
    }
  },
  
  // Kiểm tra câu trả lời
  checkAnswer: async (answerData) => {
    try {
      console.log("Gửi request kiểm tra câu trả lời behavior với data:", answerData);
      const response = await api.post('/behavior/check-answer', answerData);
      return {
        success: response.data.success,
        data: response.data.data,
        message: response.data.message
  ...
  submitGameResult: async (gameStats) => {
    try {
      const response = await api.post('/behavior/submit-result', gameStats);
      return {
        success: response.data.success,
        data: response.data.data,
        message: response.data.message
      ...
      console.error("Lỗi khi gửi kết quả trò chơi:", error);
      return handleError(error);
    }
  }
};
