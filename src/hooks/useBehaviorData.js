...
/**
 * Hook sử dụng API để lấy và quản lý dữ liệu câu hỏi hành vi
...
const useBehaviorData = () => {
  ...
  const fetchQuestions = useCallback(async (page = 1, limit = 50) => {
    ...
    try {
      const response = await behaviorAPI.getQuestions(page, limit);
      
      if (response.success) {
        setQuestions(response.data.questions);
        
        // Trích xuất danh sách các category
      ...
      console.error('Lỗi khi lấy câu hỏi hành vi:', err);
    } finally {
      setLoading(false);
    }
  }, []);
   ...
   */
  const getRandomQuestion = useCallback(async (category = null) => {
    setLoading(true);
    setError(null);
    
    try {
      const response = await behaviorAPI.getRandomQuestion(usedQuestions, category);
